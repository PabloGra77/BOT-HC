...
try:
    from PyQt6.QtWidgets import (
        ...
        QListWidget, QListWidgetItem, QStatusBar, QFileDialog
    )
...
class HCTab(QWidget):
    ...
    def _build_ui(self):
        lay = QVBoxLayout(self)
        grp = QGroupBox("Descargar Historias Clinicas")
        lay.addWidget(grp)
        form = QFormLayout(grp)
    ...
    def _run(self):
        if not self.txt_id.text().strip():
            ...
            return
        self.log.append(f"[INFO] Descargando HC para paciente {self.txt_id.text()}")
        self.log.append("[NOTA] Conectar con bot_engine.Bot para ejecucion real.")
